...
console.log(`- API: http://${primaryIp}:${serverPort}/api/products`);
console.log(`- Root: http://${primaryIp}:${serverPort}/`);
...
if (localIps.all.length > 1) {
  ...
  localIps.all
    ...
    .forEach((ip) => {
      console.log(`- API: http://${ip}:${serverPort}/api/products`);
      console.log(`- Root: http://${ip}:${serverPort}/`);
    });
}
